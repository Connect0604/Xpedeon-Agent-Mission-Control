...
### LLM Providers
`LLMExecutionService` abstracts four provider types (`Claude`, `OpenAI`, `Ollama`, `Custom`). Providers are stored in the DB and configurable at runtime via the Providers page. API keys/endpoints live on the `LLMProvider` model — not in `appsettings.json`.
